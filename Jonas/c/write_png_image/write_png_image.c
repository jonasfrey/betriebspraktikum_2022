...
int main ()
{
    ...
    for (y = 0; y < o_image.height; y++) {
        for (x = 0; x < o_image.width; x++) {
            ...
            pixel->red = (uint8_t) (f_n_random_normalized() * (pow(2, n_bits_per_color)-1));
            pixel->green = (uint8_t) (f_n_random_normalized() * (pow(2, n_bits_per_color)-1));
            pixel->blue = (uint8_t) (f_n_random_normalized() * (pow(2, n_bits_per_color)-1));
        }
    }

    /* Write the image to a file 'o_image.png'. */

    if (save_png_to_file (& o_image, "o_image.png")) {
