...
def f_o_get_debayered_pixel(
    ...
    if(s_pattern_name == "rrgb"): 
        o_debayered_pixel = numpy.array(
                [
                    (numpy.uint8(n_val4)),#b
                    (numpy.uint8(n_val3)),#g
                    (numpy.uint8((n_val1 + n_val2)/2))#r
                ]),
        dtype=numpy.uint8

    if(s_pattern_name == "bgrg"): 
        o_debayered_pixel = numpy.array(
                [
                    (numpy.uint8(n_val1)),#b
                    (numpy.uint8((n_val2 + n_val4)/2)),#g
    ...
    if(s_pattern_name == "rauschen"): 
        o_debayered_pixel = numpy.array(
                [
                    0,#((numpy.uint8(random.random() * pow(2,8)))), # pow() == Funktion mit Basis 2 und Exponent 8
                    ((numpy.uint8(random.random() * pow(2,8)))), #unnötige Klammern kann  man machen
                    0,#((numpy.uint8(random.random() * pow(2,8)))),
                ],
        dtype=numpy.uint8)
